...
BIN = os.path.join(SITE, "route.bin")
...
def build():
    ...
    # Nachschlagetabelle Streckenpunkt -> (km, Hoehe) fuer den Cloudflare-Worker.
    # Die Strecke aendert sich nicht, die Datei wird nur einmal gebraucht.
    if not os.path.exists(BIN):
        import struct
        with open(BIN + ".tmp", "wb") as fh:
            for pt in r71:
                fh.write(struct.pack("<HH", min(65535, int(round(pt[2] * 10))),
                                     max(0, min(65535, int(round(pt[3]))))))
